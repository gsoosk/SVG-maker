#include "Rectangle.h"
using namespace std;
Rectangle :: Rectangle(string _name, Group* _group, vector<vector<string> > _points): Shape(_name, _group)
{
	points = _points;
}

bool Rectangle :: does_it_match_type(string typeName)
{
	return typeName == "rectangle";
}
vector<vector<string> > Rectangle :: get_att_and_merge_from_parents()
{
...
void Rectangle :: change_att(string att, string value)
{
	if(att == "points")
	{
